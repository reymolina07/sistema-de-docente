...
function Header()
{
	...
	$this->Cell(197,12,'hola',0,1,'C');
	$this->Image('membrete1.png',15,10,185);
	$this->Cell(197,4,'REPUBLICA DE EL SALVADOR',0,1,'C');
	$this->Cell(197,4,'MINISTERIO DE EDUACION DE EL SALVADOR',0,1,'C');
        $this->Cell(197,4,'Centro Escolar Canton San Sebastian Arriba"',0,1,'C');
	$this->Cell(197,4,'MUNICIPIO DE SANTIAGO NONUALCO',0,1,'C');$this->Ln();
        $this->SetFont('Arial','B',12);
        $this->Cell(197,4, 'REPORTE GENERAL DE ESTUDIANTES REGISTRADOS',0,1,'C'); 
        $this->SetFont('Arial','',10);	
      //  $this->Cell(197,4,"Generado el ".date("j-m-y"),0,0,'C');
        $this->Cell(197,4,"Generado el ".$date,0,0,'C');
 ...
 function cabeceraHorizontal($cabecera)
    {
        ...
        $this->RoundedRect($ejeX, 50, 10, 7, 2, 'FD');
        $this->CellFitSpace(10,7, 'N',0, 0 , 'C');
        $ejeX = $ejeX + 10;

        $this->RoundedRect($ejeX, 50, 34, 7, 2, 'FD');
        $this->CellFitSpace(34,7, 'CEDULA ESCOLAR',0, 0 , 'C');
        $ejeX = $ejeX + 34;

        $this->RoundedRect($ejeX, 50, 48, 7, 2, 'FD');
        $this->CellFitSpace(48,7, 'NOMBRES',0, 0 , 'C');
        $ejeX = $ejeX + 48;

        $this->RoundedRect($ejeX, 50, 48, 7, 2, 'FD');
        $this->CellFitSpace(48,7, 'APELLIDOS',0, 0 , 'C');
        $ejeX = $ejeX + 48;

        $this->RoundedRect($ejeX, 50, 28, 7, 2, 'FD');
        $this->CellFitSpace(28,7, 'SEXO',0, 0 , 'C');
        $ejeX = $ejeX + 28;

        $this->RoundedRect($ejeX, 50, 27, 7, 2, 'FD');
        $this->CellFitSpace(27,7, 'F. NACIMIENTO',0, 0 , 'C');
    ...
    function datosHorizontal($datos)
    {
        ...
        foreach($datos as $fila)
        {
            ...
            $this->RoundedRect(10, $ejeY, 195, 5, 2, $letra);
            $this->CellFitSpace(10,5, utf8_decode($cont),'R', 0 , 'C' );
            $this->CellFitSpace(34,5, utf8_decode($fila['cedulaalu']),'R', 0 , 'C' );
            $this->CellFitSpace(48,5, utf8_decode($fila['nombrealu']),'R', 0 , 'C' );
            $this->CellFitSpace(48,5, utf8_decode($fila['apllidoalu']),'R', 0 , 'C' );
            $this->CellFitSpace(28,5, utf8_decode($fila['sexoalu']),'R', 0 , 'C');
            $this->CellFitSpace(27,5, utf8_decode($fila['fechanacalu']),0, 0 , 'C' );
